...
HRESULT CreateMachineAccount(_In_ LPCWSTR lpwComputername, _In_ LPCWSTR lpwPassword) {
	...
	// emp3r0r always packs every declared BOF argument, so an omitted
	// password arrives as an empty string rather than NULL. Treat both as
	// "generate a random password" to preserve the original behaviour.
	if (lpwPassword == NULL || lpwPassword[0] == L'\0') { // Generate random password
