...
BOOL EnumPeb(_In_ HANDLE hProcess) {
	...
	if (IsProcessWoW64(hProcess) && (!IsProcessWoW64(NtCurrentProcess()))) {
		BeaconPrintToStreamW(L"%-18ls 0x%008llX\n", L"    PEB Address:", pbi.PebBaseAddress);
	}
	else{
	...
	status = ZwReadVirtualMemory(hProcess, peb.Ldr, &pld, sizeof(pld), NULL);
	if (status != STATUS_SUCCESS) {
		return FALSE;
	}

	LPVOID pStartModuleInfo = pld.InLoadOrderModuleList.Flink;
	...
	do
	{
		status = ZwReadVirtualMemory(hProcess, pNextModuleInfo, &ldte, sizeof(ldte), NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}

		status = ZwReadVirtualMemory(hProcess, (LPVOID)ldte.FullDllName.Buffer, g_lpwReadBuf, ldte.FullDllName.Length, NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}

		if (dwCount == 0) {
			g_lpwReadBuf[ldte.FullDllName.Length / sizeof(WCHAR)] = L'\0';
			BeaconPrintToStreamW(L"%-19ls %ls\n", L"\n[M] ModuleList:", g_lpwReadBuf);
			dwCount++;
		}
		else {
			g_lpwReadBuf[ldte.FullDllName.Length / sizeof(WCHAR)] = L'\0';
...
BOOL EnumPebFromWoW64(_In_ HANDLE hProcess) {
	...
	if (IsProcessWoW64(hProcess)) {
		BeaconPrintToStreamW(L"%-18ls 0x%008llX\n", L"    PEB Address:", pbi64.PebBaseAddress);
	}
	else{
		BeaconPrintToStreamW(L"%-18ls 0x%016llX\n", L"    PEB Address:", pbi64.PebBaseAddress);
	}
	...
	status = NtWow64ReadVirtualMemory64(hProcess, peb64.Ldr, &pld64, sizeof(pld64), NULL);
	if (status != STATUS_SUCCESS) {
		return FALSE;
	}

	ULONG64 pStartModuleInfo = pld64.InLoadOrderModuleList.Flink;
	ULONG64 pNextModuleInfo = pld64.InLoadOrderModuleList.Flink;

	DWORD dwCount = 0;

	do
	{
		status = NtWow64ReadVirtualMemory64(hProcess, pNextModuleInfo, &ldte64, sizeof(ldte64), NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}

		status = NtWow64ReadVirtualMemory64(hProcess, ldte64.FullDllName.Buffer, g_lpwReadBuf, ldte64.FullDllName.Length, NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}

		if (dwCount == 0) {
			g_lpwReadBuf[ldte64.FullDllName.Length / sizeof(WCHAR)] = L'\0';
			BeaconPrintToStreamW(L"%-19ls %ls\n", L"\n[M] ModuleList:", g_lpwReadBuf);
			dwCount++;
		}
		else {
		...
		MSVCRT$memset(g_lpwReadBuf, 0, MAX_STRING * sizeof(WCHAR));
		pNextModuleInfo = ldte64.InLoadOrderLinks.Flink;
...
BOOL GetTcpSessions(_In_ DWORD ProcessId) {
	BOOL bResult = FALSE;
...
BOOL GetTcp6Sessions(_In_ DWORD ProcessId) {
	BOOL bResult = FALSE;
...
VOID Psm(_In_ DWORD dwPid) {
	...
	do {
		...
		if (HandleToULong(pProcInfo->ProcessId) == dwPid) {
			...
			GetTcpSessions(HandleToULong(pProcInfo->ProcessId));
			GetTcp6Sessions(HandleToULong(pProcInfo->ProcessId));
...
VOID go(_In_ PCHAR Args, _In_ ULONG Length) {
	...
	if (dwPid != 0) {
		...
		BeaconPrintf(CALLBACK_ERROR, "Invalid ProcessID.");
	}
