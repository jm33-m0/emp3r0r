...
def parse_args():
    ...
    if len(sys.argv) < 5:
        print(f"Usage: {sys.argv[0]} <stub.bin> <stub.elf> <stager.bin> <out.bin>",
              file=sys.stderr)
        sys.exit(1)
    return sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]


def patch_and_write_packed(stub_path, elf_path, payload, packed_payload, out_path,
                           key=b'', algo_name='packer'):
    ...
    print(f"packed({algo_name}): {len(final_blob)} bytes "
