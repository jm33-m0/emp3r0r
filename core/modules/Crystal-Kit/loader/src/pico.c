...
DECLSPEC_IMPORT LPVOID WINAPI KERNEL32$HeapReAlloc(HANDLE, DWORD, LPVOID,
                                                   SIZE_T);
...
LPVOID WINAPI _HeapAlloc(HANDLE hHeap, DWORD dwFlags, SIZE_T dwBytes) {
  ...
  if (dwBytes >= 256 && result != NULL &&
      g_memory.Heap.Count < MAX_HEAP_RECORDS) {
...
LPVOID WINAPI _HeapReAlloc(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem,
                           SIZE_T dwBytes) {
  FUNCTION_CALL call = {0};
