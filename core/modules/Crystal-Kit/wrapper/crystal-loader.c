...
EXPORT int __cdecl LoadAndRun(char *argsBuffer, uint32_t bufferSize, goCallback callback)
{
    ...
    void *mem = VirtualAlloc(NULL, (SIZE_T)picoSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (mem == NULL)
    {
    ...
    if (runtimeArgs != NULL)
    {
        ...
        rs.hRead = hRead;
        rs.cap = PIPE_OUT_CAP;
        rs.buf = (char *)VirtualAlloc(NULL, PIPE_OUT_CAP, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        if (rs.buf == NULL)
        {
            CloseHandle(hWrite);
            CloseHandle(hRead);
