...
void loader_main(long *sp) {
  ...
  void *dl_mem = (void *)mmap(NULL, downloader_bin_len, PROT_READ | PROT_WRITE,
                              MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (dl_mem == MAP_FAILED)
    exit(1);
  ...
  decode_config_string(dl_module_path, encoded_lib_path,
                       sizeof(dl_module_path));
#endif
  ...
  void *dl_exec =
      (void *)mmap(NULL, dl_map_len, PROT_READ | PROT_WRITE | PROT_EXEC,
                   MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  for (size_t i = 0; i < (size_t)downloader_bin_len; i++) {
