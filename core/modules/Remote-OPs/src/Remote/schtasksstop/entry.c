...
DWORD stopTask(const wchar_t * server, const wchar_t * taskname)
{
	...
	internal_printf("Stop task returned success.\n");
...
) 
{
	...
	internal_printf("stopTask hostname:%S taskname:%S\n", 
		hostname, taskname );

	dwErrorCode = stopTask(hostname, taskname);
	if(ERROR_SUCCESS != dwErrorCode)
	{
		BeaconPrintf(CALLBACK_ERROR, "stopTask failed: %lX\n", dwErrorCode);
		goto go_end;
	}
...
int main(int argc, char ** argv)
{
	...
	internal_printf("stopTask lpcswzHostName:%S lpcswzTaskName:%S\n", 
		lpcswzHostName, lpcswzTaskName );

	dwErrorCode = stopTask(lpcswzHostName, lpcswzTaskName);
	if(ERROR_SUCCESS != dwErrorCode)
	{
		BeaconPrintf(CALLBACK_ERROR, "stopTask failed: %lX\n", dwErrorCode);
		goto main_end;
