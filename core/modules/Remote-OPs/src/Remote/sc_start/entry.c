...
DWORD start_service(const char* Hostname, const char* cpServiceName)
{
...
) 
{
	...
	internal_printf("start_service:\n");
	internal_printf("  hostname:    %s\n", hostname);
	internal_printf("  servicename: %s\n", servicename);

	dwErrorCode = start_service(hostname, servicename);
