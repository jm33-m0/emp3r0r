# Starlark implementation of netlocalgroup/entry.c
...
def list_local_groups(server=None):
    ...
    if stat == 0 or stat == 234:
        ...
        # LOCALGROUP_INFO_1 size on x64 is 16 bytes: lgrpi1_name (0,8), lgrpi1_comment (8,8)
        ...
        print("[-] NetLocalGroupEnum failed: status %d (Error %d: %s)" % (stat, err_code, err_msg))
...
def list_local_group_members(group_name, server=None):
    if not group_name:
    ...
    if stat == 0 or stat == 234:
        ...
        print("Members of Local Group '%s':" % group_name)
        print("--------------------------------")
        for i in range(count):
        ...
        err_code = res.get("err_code", 0)
        err_msg = res.get("error", "")
        print("[-] NetLocalGroupGetMembers failed: status %d (Error %d: %s)" % (stat, err_code, err_msg))
...
def main(*args):
    ...
    if group:
        return list_local_group_members(group, server)
    else:
