...
def get_current_token():
    ...
    # Try the thread token first — when the agent has stolen a token and is
    # impersonating via ExecuteAsToken / runWithToken, the thread token
    # reflects the stolen identity while the process token remains unchanged.
    res = win_call("kernel32.dll", "GetCurrentThread")
    h_thread = res["r1"]

    # OpenAsSelf = FALSE (0): use the thread's impersonation context for the
    # access check, so we can open the impersonation token itself.
    res = win_call(
        "advapi32.dll", "OpenThreadToken", h_thread, TOKEN_QUERY, 0, h_token_ptr
    )
    if res["r1"] != 0:
    ...
    # No thread token — fall back to the primary process token.
...
def lookup_privilege_name_ffi(luid_ptr):
    ...
    name_buf = win_alloc(name_size * 2)
    res = win_call(
        "advapi32.dll", "LookupPrivilegeNameW", 0, luid_ptr, name_buf, cch_name_ptr
    )
...
def do_whoami_groups():
    ...
    for i in range(group_count):
        ...
        str_sid_ptr_ptr = win_alloc(8)
        win_call(
            "advapi32.dll", "ConvertSidToStringSidW", group_sid_ptr, str_sid_ptr_ptr
        )
        str_sid_ptr = read_ptr(str_sid_ptr_ptr, 0)
        win_free(str_sid_ptr_ptr)
