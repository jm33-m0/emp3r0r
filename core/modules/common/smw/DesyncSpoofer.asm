...
.skip_thread_root:
    ...
    ;   Set the pointer to the function to call in RSI and stash the syscall
    ;   number in R11 — both volatile registers — before parameter_handler
    ;   clobbers RCX (with arg1) and RAX (transiently).
    mov     r11, [rcx + SMW_SSN]
    mov     rsi, [rcx + SMW_SPOOF_FUNC]
...
execute:
    ;   Indirect-syscall target (ntdll "syscall; ret" gadget):
    ;   - R10 = arg1: the syscall instruction clobbers RCX (it saves RIP
    ;     there) and the kernel reads arg1 from R10 — ntdll stubs do
    ;     "mov r10, rcx" for exactly this reason.
    ;   - EAX = SSN: the syscall number.
    ;   - Jump via RSI (volatile; harmless for plain API targets).
    mov     r10, rcx
    mov     eax, r11d
    jmp     rsi
