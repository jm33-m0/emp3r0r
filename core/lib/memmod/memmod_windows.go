...
func (head *addressList) free(table *ntsyscall.SyscallTable) {
	for node := head; node != nil; node = node.next {
		address := node.address
		var regionSize uintptr
		_, _ = ntsyscall.NtFreeVirtualMemory(table, windows.CurrentProcess(), &address, &regionSize, windows.MEM_RELEASE)
	}
}
...
func (module *Module) copySections(address, size uintptr, oldHeaders *IMAGE_NT_HEADERS) error {
	...
	for i := range sections {
		if sections[i].SizeOfRawData == 0 {
			...
			regionSize := uintptr(sectionSize)
			status, allocErr := ntsyscall.NtAllocateVirtualMemory(module.table, windows.CurrentProcess(), &dest, 0, &regionSize, windows.MEM_COMMIT, windows.PAGE_READWRITE)
			if allocErr != nil || status != ntsyscall.STATUS_SUCCESS {
				return fmt.Errorf("Error allocating section: 0x%08X, %w", status, allocErr)
			}
		...
		dest := module.codeBase + uintptr(sections[i].VirtualAddress)
		regionSize := uintptr(sections[i].SizeOfRawData)
		status, allocErr := ntsyscall.NtAllocateVirtualMemory(module.table, windows.CurrentProcess(), &dest, 0, &regionSize, windows.MEM_COMMIT, windows.PAGE_READWRITE)
		if allocErr != nil || status != ntsyscall.STATUS_SUCCESS {
...
func (module *Module) finalizeSection(sectionData *sectionFinalizeData) error {
	...
	if (sectionData.characteristics & IMAGE_SCN_MEM_DISCARDABLE) != 0 {
		...
		if sectionData.address == sectionData.alignedAddress &&
			...
			_, _ = ntsyscall.NtFreeVirtualMemory(module.table, windows.CurrentProcess(), &address, &size, windows.MEM_DECOMMIT)
		}
		return nil
	}
	...
	address := sectionData.address
	size := sectionData.size
	status, err := ntsyscall.NtProtectVirtualMemory(module.table, windows.CurrentProcess(), &address, &size, protect, &oldProtect)
	if err != nil || status != ntsyscall.STATUS_SUCCESS {
		return fmt.Errorf("Error protecting memory page: 0x%08X, %w", status, err)
	}
...
func hookRtlPcToFileHeader(table *ntsyscall.SyscallTable) error {
	...
	var oldProtect uint32
	thunkAddr := uintptr(unsafe.Pointer(thunk))
	thunkSize := uintptr(unsafe.Sizeof(*thunk))
	status, err := ntsyscall.NtProtectVirtualMemory(table, windows.CurrentProcess(), &thunkAddr, &thunkSize, windows.PAGE_READWRITE, &oldProtect)
	if err != nil || status != ntsyscall.STATUS_SUCCESS {
		return fmt.Errorf("NtProtectVirtualMemory: 0x%08X, %w", status, err)
	}
	...
	*thunk = windows.NewCallback(func(pcValue uintptr, baseOfImage *uintptr) uintptr {
	...
	thunkAddr = uintptr(unsafe.Pointer(thunk))
	thunkSize = uintptr(unsafe.Sizeof(*thunk))
	status, err = ntsyscall.NtProtectVirtualMemory(table, windows.CurrentProcess(), &thunkAddr, &thunkSize, oldProtect, &oldProtect)
	if err != nil || status != ntsyscall.STATUS_SUCCESS {
		return fmt.Errorf("NtProtectVirtualMemory (restore): 0x%08X, %w", status, err)
	}
	return nil
}
...
func LoadLibrary(data []byte) (module *Module, err error) {
	...
	if table == nil {
		return nil, errors.New("syscall table not initialized")
	}
	...
	module = &Module{isDLL: (oldHeader.FileHeader.Characteristics & IMAGE_FILE_DLL) != 0, table: table}
	defer func() {
		if err != nil {
			module.Free()
			module = nil
	...
	baseAddr := oldHeader.OptionalHeader.ImageBase
	...
	if allocErr != nil || status != ntsyscall.STATUS_SUCCESS {
		...
		status, allocErr = ntsyscall.NtAllocateVirtualMemory(module.table, windows.CurrentProcess(), &baseAddr, 0, &regionSize, windows.MEM_RESERVE|windows.MEM_COMMIT, windows.PAGE_READWRITE)
		if allocErr != nil || status != ntsyscall.STATUS_SUCCESS {
			err = fmt.Errorf("Error allocating code: 0x%08X, %w", status, allocErr)
			return
		}
	}
	...
	headers := module.codeBase
	headerSize := uintptr(oldHeader.OptionalHeader.SizeOfHeaders)
	status, allocErr = ntsyscall.NtAllocateVirtualMemory(module.table, windows.CurrentProcess(), &headers, 0, &headerSize, windows.MEM_COMMIT, windows.PAGE_READWRITE)
	if allocErr != nil || status != ntsyscall.STATUS_SUCCESS {
		err = fmt.Errorf("Error allocating headers: 0x%08X, %w", status, allocErr)
		return
	}
	...
	haveHookedRtlPcToFileHeader.Do(func() {
		hookRtlPcToFileHeaderResult = hookRtlPcToFileHeader(module.table)
	})
...
func (module *Module) Free() {
	...
	if module.codeBase != 0 {
		if module.table != nil {
			...
			var regionSize uintptr
			_, _ = ntsyscall.NtFreeVirtualMemory(module.table, windows.CurrentProcess(), &address, &regionSize, windows.MEM_RELEASE)
		}
		module.codeBase = 0
	}
	if module.blockedMemory != nil {
		if module.table != nil {
			module.blockedMemory.free(module.table)
		}
