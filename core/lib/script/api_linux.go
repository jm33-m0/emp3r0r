...
// Common Linux syscall name to number lookup table
var syscallNames = map[string]uintptr{
	"read":                   unix.SYS_READ,
...
func parseSyscallNum(val starlark.Value) (uintptr, error) {
	...
	case starlark.String:
		...
		if num, ok := syscallNames[cleanStr]; ok {
			return num, nil
		}
		return 0, fmt.Errorf("unknown syscall name: %s", str)
	default:
