...
func TLSServer() {
	...
	err := http.ListenAndServeTLS(fmt.Sprintf(":%s", RuntimeConfig.CCPort), EmpRoot+"/emp3r0r-cert.pem", EmpRoot+"/emp3r0r-key.pem", nil)
	if err != nil {
		log.Fatalf(color.RedString("Start HTTPS server: %v", err))
	}
}
