...
func CmdGenerateAgent(cmd *cobra.Command, args []string) {
	...
	if len(config_payload) < len(emp3r0r_def.AgentConfig) {
		...
		logging.Errorf("Config payload is too large, %d bytes, max %d bytes", len(config_payload), len(emp3r0r_def.AgentConfig))
		return
	}
	// fill in
...
func readAndEncryptConfig() ([]byte, error) {
	...
	// encrypt
	encryptedJSONBytes, err := emp3r0r_crypto.AES_GCM_Encrypt(emp3r0r_def.OneTimeMagicBytes, jsonBytes)
	if err != nil {
