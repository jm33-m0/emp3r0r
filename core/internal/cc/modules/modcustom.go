...
// readModConfigs parses a module manifest. The implementation lives in
// lib/modconfig so cmd/bofrunner can reuse the exact same parameter typing
// and COFF packing rules; this wrapper keeps the package-local call sites.
func readModConfigs(file string) ([]*def.ModuleConfig, error) {
	return modconfig.ReadConfigs(file)
}
...
// resolveInvocation turns operator-supplied flag values into a fully
// resolved invocation. The implementation lives in lib/modconfig (shared
// with cmd/bofrunner); this wrapper keeps the package-local call sites.
func resolveInvocation(config *def.ModuleConfig, flags map[string]string) (def.ResolvedInvocation, error) {
	return modconfig.ResolveInvocation(config, flags)
}
