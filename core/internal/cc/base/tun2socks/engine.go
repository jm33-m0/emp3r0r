...
func Start(cfg Config) (*Engine, error) {
	...
	if err != nil {
		_ = device.Close()
		return fail(fmt.Errorf("new gvisor stack: %w", err))
	}
...
// Close stops the stack and the TUN device. sing-tun removes the routes,
// rules and the device itself, so nothing lingers after shutdown.
