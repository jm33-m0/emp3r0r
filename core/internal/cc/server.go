...
func (sh *StreamHandler) proxyHandler(wrt http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		CliPrintError("proxyHandler: failed creating connection from %s: %s", req.RemoteAddr, err)
	...
	sh.H2x.Conn = conn
	CliPrintWarning("Got a proxy connection from %s", req.RemoteAddr)

	defer func() {
		err = sh.H2x.Conn.Close()
		if err != nil {
			CliPrintError("proxyHandler failed to close connection: " + err.Error())
		}
...
func TLSServer() {
	...
	http.HandleFunc("/"+tun.ProxyAPI, ProxyStream.proxyHandler)
