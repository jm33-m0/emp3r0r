...
func VaccineHandler(download_addr, checksum string) (out string) {
	...
	if util.IsFileExist(UtilsArchivePath) && crypto.SHA256SumFile(UtilsArchivePath) == checksum {
		logging.Printf("%s already exists, skipping download", UtilsArchivePath)
	}
	...
	if err != nil {
		...
		out = logging.Sprintf("[-] Download error: %v", err)
		return
	}
	...
	if err = util.Unarchive(UtilsArchivePath, extractPath); err != nil {
		logging.Printf("Unarchive: %v", err)
		return logging.Sprintf("Unarchive: %v", err)
	}
	logging.Printf("%s extracted", UtilsArchive)

	// unpack libs
	if err = util.Unarchive(common.RuntimeConfig.UtilsPath+"/libs.tar.xz", extractPath); err != nil {
		logging.Printf("Unarchive: %v", err)
		out = logging.Sprintf("Unarchive libs: %v", err)
		return
	}
	logging.Println("Libs configured")
	defer os.Remove(common.RuntimeConfig.UtilsPath + "/libs.tar.xz")

	// extract python3.9.tar.xz
	logging.Printf("Pre-set Python environment: %s, %s, %s", PythonArchive, PythonLib, PythonPath)
	os.RemoveAll(PythonLib)
	logging.Printf("Found python archive at %s, trying to configure", PythonArchive)
	defer os.Remove(PythonArchive)
	if err = util.Unarchive(PythonArchive, extractPath); err != nil {
		out = logging.Sprintf("Unarchive python libs: %v", err)
		logging.Print(out)
		return
	}
	...
	if err != nil {
		out = logging.Sprintf("Write python launcher: %v", err)
	}
	logging.Println("Python configured")

	// fix ELFs
	files, err := os.ReadDir(common.RuntimeConfig.UtilsPath)
	if err != nil {
		return logging.Sprintf("Fix ELFs: %v", err)
	}
	for _, f := range files {
		...
		if f.Name() == "patchelf" {
			...
			if err != nil {
				out = logging.Sprintf("%s, %s: %v", out, fpath, err)
			}
		}
	}

	logging.Println("ELFs configured")
